          ...
          $uibModal.open(
            {
              ...
              controller: function ($scope, $uibModalInstance, allRoles, roleUsers, globalProjectRoles, permissionRoleIndex) {
                ...
                var setPermissionRole = function (system, permission, role, value) {
                  if (!system.permission[permission]) {
                ...
                s.init = function (roleName, showPermissionView) {
                  ...
                  s.allPermissions = _.keys(scope.object.userCan).filter(function (e) {
                    return e !== 'public';
                  });
                  ...
                  s.allRoles = _.sortBy(s.allRoles, function (r) {
                    return r.toLowerCase();
                  });
                  s.allPermissions = _.sortBy(s.allPermissions, function (r) {
                    return r.toLowerCase();
                  });
                };
                ...
                s.ok = function () {
                  $uibModalInstance.close({
                    resource: s.object._id,
                    data: s.permissionRoleIndex
                  });
                };
              }
            })
          ...
          $uibModal.open(
            {
              ...
              controller: function ($scope, $uibModalInstance, allRoles, userRoleIndex, userList, globalProjectRoles) {
                ...
                var contextRoles = _.filter(allRoles, function (r) {
                  if (r === 'public') {
                    return;
                  } else if (_.find(globalProjectRoles, function (gpr) {
                    return gpr === r
                  }) === undefined) {
                    return r;
                  }
                ...
                $scope.$on('USER_SEARCH_CHOOSER_SELECTED', function (e, data) {
                  if (data && data.users && data.users.length > 0) {
                    _.forEach(data.users, function (user) {
                      var item = _.find(s.allUsers, function (o) {
                ...
                s.init = function (users, currentRoleName, currentUserName, showUserView, filterGlobalUsers) {
                  ...
                  var allUserz = _.filter(users, function (u) {
                    return globalUsers.indexOf(u.username) < 0;
                  });

                  // sort alpha...
                  s.allRoles = _.sortBy(allRolez, function (r) {
                    return r.toLowerCase();
                  });
                  ...
                  var selectedUser = _.find(s.allUsers, function (o) {
                    return o.username === currentUserName;
                  });
                  ...
                  _.each(s.userRoleIndex.user, function (roles, user) {
                    _.each(roles, function (value, role) {
                      if (value) {
                        ...
                        if (contextRole) {
                          contextInsertData.push({
                            context: s.context._id,
                            user: user,
                            role: role
                          });
                        } else {
                          globalInsertData.push({
                            context: s.context._id,
                            user: user,
                            role: role
                          });
                    ...
                    ProjectModel.picklist()
                      .then(function (data) {
                        _.each(data, function (p) {
                          projectRolesDelete.push({
                            _id: p._id,
                            name: p.name,
                            data: {
                              context: p._id,
                              roles: globalProjectRoles
                            }
                          });
                          var piddy = {
                            _id: p._id,
                        ...
                        return AccessModel.purgeUserRoles({
                          context: s.context._id,
                          roles: contextRoles
                        });
          ...
          $uibModal.open(
            {
              ...
              controller: function ($rootScope, $scope, $uibModalInstance, allUsers) {
                ...
                s.ok = function () {
                  $rootScope.$broadcast('NEW_USER_ADDED_TO_CONTEXT', {
                    user: s.currentUser
                  });
                  $uibModalInstance.close();
                };
          ...
          $uibModal.open(
            {
              ...
              controller: function ($scope, $uibModalInstance, allRoles) {
                ...
                var sortedRoles = _.sortBy(nonPublicRoles, function (r) {
                  return r.toLowerCase();
                });
