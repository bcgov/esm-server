...
//
// -----------------------------------------------------------------------------------
controllerModalDayCalculator.$inject = ['$scope', '$uibModalInstance', 'moment'];
/* @ngInject */
function controllerModalDayCalculator($scope, $uibModalInstance, moment) {
...
function controllerRequirementCalculation($scope, _) {
  ...
  var indicateRequirements = function() {
    if (reqChecklist.requiredList && reqChecklist.project) {
      _.each(reqChecklist.requiredList, function(req) {
